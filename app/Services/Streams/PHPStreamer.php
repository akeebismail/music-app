...
class PHPStreamer extends Streamer implements StreamInterface{
    ...
    public function stream()
    {
        ...
        $fileSize = filesize($this->_song->path);
        ...
        if ($range) {
            ...
            // No range requested
            $length = filesize($this->_song->path);
            $partial = false;
        }
        ...
        header('Content-Disposition: attachment; filename="'.basename($this->_song->path).'"');
        ...
        if ($partial) {
            ...
            abort_unless($fp = fopen($this->_song->path, 'r'), 500);
